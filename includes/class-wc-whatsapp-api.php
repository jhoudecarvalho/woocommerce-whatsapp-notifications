...
class WC_WhatsApp_API {
	 ...
	 * @param array|null  $media_data Dados da mídia (imagem) a ser enviada (opcional).
	 * @return bool|WP_Error True se enviado com sucesso, WP_Error em caso de erro.
	 */
	public function send_message( $number, $message, $custom_endpoint = null, $body_format = null, $skip_rate_limit = false, $media_data = null ) {
		...
		// Adiciona externalKey se fornecido
		if ( ! empty( $media_data['external_key'] ) ) {
			$data['externalKey'] = sanitize_text_field( $media_data['external_key'] );
		}

		// Adiciona mediaMessage se imagem fornecida
		if ( ! empty( $media_data ) && isset( $media_data['image_path'] ) && ! empty( $media_data['image_path'] ) ) {
			$image_base64 = $this->convert_image_to_base64( $media_data['image_path'] );
			
			if ( is_wp_error( $image_base64 ) ) {
				$this->logger->error( 'Erro ao converter imagem para base64', array( 'error' => $image_base64->get_error_message() ) );
				// Continua sem imagem se houver erro
			} else {
				// Obtém nome do arquivo e mime type
				$file_name = 'image.jpg';
				$mime_type = 'image/jpeg';
				
				// Se for um attachment ID do WordPress
				if ( is_numeric( $media_data['image_path'] ) ) {
					$attachment_id = intval( $media_data['image_path'] );
					$file_path = get_attached_file( $attachment_id );
					
					if ( $file_path ) {
						$file_info = pathinfo( $file_path );
						$file_name = isset( $file_info['basename'] ) ? $file_info['basename'] : 'image.jpg';
					}
					
					$mime_type = get_post_mime_type( $attachment_id );
					if ( ! $mime_type ) {
						$mime_type = $this->get_image_mime_type( $file_path ? $file_path : $media_data['image_path'] );
					}
				} else {
					// É um caminho de arquivo
					$file_info = pathinfo( $media_data['image_path'] );
					$file_name = isset( $file_info['basename'] ) ? $file_info['basename'] : 'image.jpg';
					$mime_type = $this->get_image_mime_type( $media_data['image_path'] );
				}
				
				$data['mediaMessage'] = array(
					'mediaType' => $mime_type,
					'fileName'  => $file_name,
					'media'     => $image_base64,
				);
	 ...
	 *
	 * @param string $image_path Caminho completo da imagem.
	 * @return string|WP_Error Base64 da imagem ou WP_Error em caso de erro.
	 */
	private function convert_image_to_base64( $image_path ) {
		// Se for um attachment ID do WordPress, obtém o caminho
		if ( is_numeric( $image_path ) ) {
			$attachment_id = intval( $image_path );
			$file_path = get_attached_file( $attachment_id );
			
			if ( ! $file_path || ! file_exists( $file_path ) ) {
				return new WP_Error( 'image_not_found', __( 'Imagem não encontrada.', 'wc-whatsapp-notifications' ) );
			}
		...
		if ( ! file_exists( $image_path ) ) {
			return new WP_Error( 'image_not_found', __( 'Arquivo de imagem não encontrado.', 'wc-whatsapp-notifications' ) );
		}
		
		// Verifica se é uma imagem válida
		$image_info = @getimagesize( $image_path );
		if ( false === $image_info ) {
			return new WP_Error( 'invalid_image', __( 'Arquivo não é uma imagem válida.', 'wc-whatsapp-notifications' ) );
		}
		
		// Lê o arquivo e converte para base64
		$image_data = file_get_contents( $image_path );
		if ( false === $image_data ) {
			return new WP_Error( 'read_error', __( 'Erro ao ler arquivo de imagem.', 'wc-whatsapp-notifications' ) );
		}
	 ...
	 *
	 * @param string $image_path Caminho completo da imagem.
	 * @return string MIME type da imagem.
	 */
	private function get_image_mime_type( $image_path ) {
		// Se for um attachment ID do WordPress, obtém o mime type
		if ( is_numeric( $image_path ) ) {
			$attachment_id = intval( $image_path );
			$mime_type = get_post_mime_type( $attachment_id );
