...
class WC_WhatsApp_Handler {
	...
	/**
	 * Obtém meta do pedido (compatível com HPOS)
	 *
	...
	private function get_order_meta( $order, $meta_key, $single = true ) {
		...
		$order_id = is_numeric( $order ) ? $order : 0;
		if ( $order_id ) {
			// Fallback para get_post_meta (ainda funciona com HPOS via compatibilidade)
	...
	/**
	 * Atualiza meta do pedido (compatível com HPOS)
	 *
	 * @param WC_Order|int $order Objeto do pedido ou ID do pedido.
	 * @param string       $meta_key Chave do meta.
	 * @param mixed        $meta_value Valor do meta.
	 * @return bool|int True se atualizado, false em caso de erro.
	 */
	private function update_order_meta( $order, $meta_key, $meta_value ) {
		...
		$order_id = is_numeric( $order ) ? $order : 0;
		if ( $order_id ) {
			// Fallback para update_post_meta (ainda funciona com HPOS via compatibilidade)
	...
	/**
	 * Remove meta do pedido (compatível com HPOS)
	 *
	 * @param WC_Order|int $order Objeto do pedido ou ID do pedido.
	 * @param string       $meta_key Chave do meta.
	 * @return bool True se removido, false em caso de erro.
	 */
	private function delete_order_meta( $order, $meta_key ) {
		...
		$order_id = is_numeric( $order ) ? $order : 0;
		if ( $order_id ) {
			// Fallback para delete_post_meta (ainda funciona com HPOS via compatibilidade)
	...
	public function handle_order_saved( $order_id, $post ) {
		...
		$last_processed_status = $this->get_order_meta( $order, '_wc_whatsapp_last_processed_status', true );
		...
		if ( $last_processed_status && $last_processed_status !== $current_status ) {
			...
			$this->update_order_meta( $order, '_wc_whatsapp_last_processed_status', $current_status );
		} else {
	...
	private function schedule_order_processing( int $order_id ): void {
		...
		$already_notified = $this->get_order_meta( $order, $notification_meta_key, true );
		...
		$processing_meta = $this->get_order_meta( $order, '_wc_whatsapp_processing', true );
		...
		$this->update_order_meta( $order, '_wc_whatsapp_processing', time() );
	...
	private function process_new_order( $order_id ) {
		...
		$this->update_order_meta( $order, '_wc_whatsapp_last_processed_status', $status );
	}
	...
	public function handle_order_status_change( $order_id, $old_status, $new_status, $order ) {
		...
		// Salva status processado para referência futura
		$this->update_order_meta( $order, '_wc_whatsapp_last_processed_status', $new_status );
	}
	...
	private function process_notification( $order_id, $old_status, $new_status, $order ) {
		...
		$notification_meta = $this->get_order_meta( $order, $notification_meta_key, true );
		...
		$processing_meta = $this->get_order_meta( $order, $processing_key, true );
		...
		$this->update_order_meta( $order, $processing_key, time() );
		...
		$this->update_order_meta( $order, $notification_meta_key, time() );
		set_transient( $notification_key, true, HOUR_IN_SECONDS );
		
		// Obtém imagem configurada para este status
		$image_id = get_option( 'wc_whatsapp_image_' . $new_status, 0 );
		$media_data = null;
		
		if ( $image_id > 0 && wp_attachment_is_image( $image_id ) ) {
			$media_data = array(
				'image_path'  => $image_id, // Passa o ID, a API vai converter
				'external_key' => 'order_' . $order_id . '_' . time(), // Chave única para rastreamento
			);
		}
		
		// Envia mensagem (não bloqueia o processo se falhar)
		$result = $this->api->send_message( $formatted_phone, $message, null, null, false, $media_data );

		if ( is_wp_error( $result ) ) {
			// Se falhou, remove a marca de notificado para permitir nova tentativa
			$this->delete_order_meta( $order, $notification_meta_key );
			...
			$this->delete_order_meta( $order, $processing_key );
	...
	public function handle_order_note_added( $note_id, $order ) {
		...
		if ( function_exists( 'wc_any_shipping_get_tracking_codes' ) ) {
			...
			if ( is_array( $tracking_codes ) && ! empty( $tracking_codes ) ) {
				...
				if ( $note ) {
					// Compatibilidade: get_comment() ainda retorna comment_content, mas wc_get_order_notes() retorna content
					$note_content = isset( $note->content ) ? $note->content : ( isset( $note->comment_content ) ? $note->comment_content : '' );
	...
	public function handle_order_updated( $order_id, $order ) {
		...
		// Usa wc_get_order_notes() para compatibilidade com WooCommerce 10.0+
		...
		if ( ! empty( $notes ) ) {
			...
			$note_content = isset( $last_note->content ) ? $last_note->content : ( isset( $last_note->comment_content ) ? $last_note->comment_content : '' );
			$tracking_code = $this->extract_tracking_code( $note_content );
		...
		foreach ( $all_notes as $note ) {
			// Compatibilidade com WooCommerce 10.0+: usa 'content' em vez de 'comment_content'
			$note_content = isset( $note->content ) ? $note->content : ( isset( $note->comment_content ) ? $note->comment_content : '' );
			$tracking_code = $this->extract_tracking_code( $note_content );
	...
	private function send_tracking_notification( $order_id, $tracking_code, $shipping_company_slug = '' ) {
		...
		// Obtém imagem configurada para rastreio
		$image_id = get_option( 'wc_whatsapp_image_tracking', 0 );
		$media_data = null;
		
		if ( $image_id > 0 && wp_attachment_is_image( $image_id ) ) {
			$media_data = array(
				'image_path'  => $image_id,
				'external_key' => 'tracking_' . $order_id . '_' . time(),
			);
		}
	...
	private function send_customer_note_notification( $order_id, $note_content ) {
		...
		// Obtém imagem configurada para observação do cliente
		$image_id = get_option( 'wc_whatsapp_image_customer_note', 0 );
		$media_data = null;
		
		if ( $image_id > 0 && wp_attachment_is_image( $image_id ) ) {
			$media_data = array(
				'image_path'  => $image_id,
				'external_key' => 'note_' . $order_id . '_' . time(),
			);
		}
